...
//   return evt
// }
function generateEvent(s: string, originEvent?: any, detail?: any) {
  const evt: any = new CustomEvent(s, { detail })
  if (originEvent) {
    evt.originEvent = originEvent
  }
  return evt
...
function getHeartCheckObj(ws: any) {
  return {
    timeout: 60000,
    timer: null,
    reset: function() {
...
export default class ReconnectingWebSocket {
  ...
  open(reconnectAttempt: boolean) {
    ...
    if (this.reconnectAttempts) {
      ...
      dispatchEvent(<any>generateEvent('connecting'))
      this.reconnectAttempts = 0
    }
    if (!this.ws) {
