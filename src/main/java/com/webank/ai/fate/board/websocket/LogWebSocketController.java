...
//        sessionMap.remove(session);
    ...
    public void onMessage(String message,
        ...
        if (file.exists()) {
            ...
            logFileService.checkSshInfo(jobIp);
            SshInfo sshInfo = sshService.getSSHInfo(jobIp);
            com.jcraft.jsch.Session jschSession = sshService.connect(sshInfo);

            Channel channel = sshService.executeCmd(jschSession, "tail -n +" + begin + " " + logPath + " | head -n " + (end - begin + 1));
    ...
    public void afterPropertiesSet() {
        this.logFileService = (LogFileService) applicationContext.getBean("logFileService");
        this.sshService = (SshService) applicationContext.getBean("sshService");
    }
